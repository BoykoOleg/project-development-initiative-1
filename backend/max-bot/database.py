...
# Подключение к БД: задай DATABASE_URL в окружении
# Пример для локальной машины: postgresql://user:password@localhost:5432/dbname
# Схема таблиц: задай MAIN_DB_SCHEMA (по умолчанию "public")
...
def fetch_db_context(conn) -> str:
    ...
    income_rows = _safe_query(conn, f"""
        SELECT COALESCE(SUM(amount), 0) FROM {t('payments')}
        WHERE created_at >= date_trunc('month', NOW())
    """, "payments_income")
    expense_rows = _safe_query(conn, f"""
        SELECT COALESCE(SUM(amount), 0) FROM {t('expenses')}
        WHERE created_at >= date_trunc('month', NOW())
    """, "expenses")
