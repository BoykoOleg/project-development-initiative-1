...
def add_works_to_wo(conn, wo_id: int, works: list) -> int:
    ...
    for w in works:
        cur.execute(f"""
            ...
            VALUES (%s, %s, %s, %s, %s, %s)
        """, (wo_id, w.get("name", "Работа"), float(w.get("price", 0)), float(w.get("qty", 1)),
              float(w.get("norm_hours", 0)), float(w.get("norm_hour_price", 0))))
        count += 1
    cur.close()
    return count
...
def create_payment_in_db(conn, amount: float, comment: str, cashbox_id: int,
                          payment_method: str = "cash", work_order_id: int = None) -> int:
    cur = conn.cursor()
    cur.execute(f"""
    ...
    if work_order_id:
        cur.execute(f"""
            UPDATE {t('work_orders')} SET status = 'issued'
...
def create_client_in_db(conn, name: str, phone: str, email: str = "", comment: str = "") -> tuple:
    ...
    if name:
        ...
        if row:
            ...
            return row[0], row[1], False
    cur.execute(f"""
        INSERT INTO {t('clients')} (name, phone, email, comment)
...
def update_client_in_db(conn, client_id: int, **fields) -> bool:
    ...
    cur = conn.cursor()
    cur.execute(f"UPDATE {t('clients')} SET {set_clause} WHERE id = %s", values)
    updated = cur.rowcount > 0
    cur.close()
    return updated


def create_car_in_db(conn, client_name: str, phone: str, brand: str, model: str = "",
                      year: str = "", vin: str = "", license_plate: str = "") -> tuple:
    ...
    """, (client_id, brand, model or "", year or None, vin or None, license_plate or None))
    car_id = cur.fetchone()[0]
    cur.close()
    return car_id, client_id
...
def update_car_in_db(conn, car_id: int, **fields) -> bool:
    ...
    cur = conn.cursor()
    cur.execute(f"UPDATE {t('cars')} SET {set_clause} WHERE id = %s", values)
    updated = cur.rowcount > 0
    cur.close()
    return updated


def create_product_in_db(conn, name: str, sku: str = "", category: str = "",
                          purchase_price: float = 0, quantity: float = 0,
                          unit: str = "шт", description: str = "") -> int:
    cur = conn.cursor()
    cur.execute(f"""
        INSERT INTO {t('products')} (name, sku, category, purchase_price, quantity, unit, description)
        VALUES (%s, %s, %s, %s, %s, %s, %s) RETURNING id
    """, (name, sku or "", category or "", purchase_price, quantity, unit or "шт", description or ""))
